...
function deployToHeroku(accessToken) {
	if (!shell.which("heroku")) {
		...
		if (shell.exec("heroku create " + "mystartupbotforviber").code !== 0) {
			shell.echo("Error: Heroku create project failed");
			shell.exit(1);
			return;
		}

		if (shell.exec("heroku config:set HEROKU_URL=https://mystartupbotforviber.herokuapp.com").code !== 0) {
...
function deployToNow(accessToken) {
	...
	if (shell.exec("now -e VIBER_PUBLIC_ACCOUNT_ACCESS_TOKEN_KEY=\'" + "471409239aa7d393-8092b2ab7f344f06-4cf82b8deb78a1b2" + "\'").code !== 0) {
